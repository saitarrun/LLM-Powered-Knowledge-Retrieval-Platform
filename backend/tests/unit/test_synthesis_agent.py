...
def make_chunk(
    ...
    return {"text": text, "metadata": {"chunk_id": chunk_id}, "db_chunk": db_chunk}


@pytest.mark.asyncio
async def test_synthesis_serializes_citation_source_fields():
    agent = SynthesisAgent()
    state = QueryState(query="What is sourced?", reranked_chunks=[make_chunk()])
...
async def test_synthesis_serializes_unavailable_citation_when_chunk_reference_missing():
    ...
    state = QueryState(
        ...
        reranked_chunks=[
            {"text": "Fallback source text.", "metadata": {"chunk_id": "missing-chunk"}, "db_chunk": None}
        ],
    )
